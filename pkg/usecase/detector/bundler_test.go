...
func TestBundler(t *testing.T) {
	...
	t.Run("detect both with 3.2.4", func(t *testing.T) {
		results, err := dt.Detect(octovy.PkgRubyGems, "somepkg", "3.2.4")
	...
	t.Run("detect CVE-2345-6789 with 3.2.5", func(t *testing.T) {
		results, err := dt.Detect(octovy.PkgRubyGems, "somepkg", "3.2.5")
		require.NoError(t, err)
	...
	t.Run("detect no vulnerability with 3.3.5", func(t *testing.T) {
		results, err := dt.Detect(octovy.PkgRubyGems, "somepkg", "3.3.5")
		require.NoError(t, err)
