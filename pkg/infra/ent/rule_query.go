...
func (rq *RuleQuery) QuerySeverity() *SeverityQuery {
	query := &SeverityQuery{config: rq.config}
...
// Returns a *NotSingularError when exactly one Rule entity is not found.
...
// Returns a *NotSingularError when exactly one Rule ID is not found.
...
func (rq *RuleQuery) Clone() *RuleQuery {
	...
	return &RuleQuery{
		...
		withSeverity: rq.withSeverity.Clone(),
...
func (rq *RuleQuery) WithSeverity(opts ...func(*SeverityQuery)) *RuleQuery {
	query := &SeverityQuery{config: rq.config}
	for _, opt := range opts {
		opt(query)
	}
	rq.withSeverity = query
	return rq
}
...
func (rq *RuleQuery) sqlAll(ctx context.Context) ([]*Rule, error) {
	...
	if query := rq.withSeverity; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Rule)
		for i := range nodes {
			if nodes[i].rule_severity == nil {
				continue
			}
			fk := *nodes[i].rule_severity
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "rule_severity" returned %v`, n.ID)
			}
			for i := range nodes {
