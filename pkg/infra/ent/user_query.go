...
type UserQuery struct {
	...
	// eager-loading edges.
	withEditedStatus *VulnStatusQuery
...
func (uq *UserQuery) QueryEditedStatus() *VulnStatusQuery {
	query := &VulnStatusQuery{config: uq.config}
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withEditedStatus: uq.withEditedStatus.Clone(),
...
// WithEditedStatus tells the query-builder to eager-load the nodes that are connected to
// the "edited_status" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithEditedStatus(opts ...func(*VulnStatusQuery)) *UserQuery {
	query := &VulnStatusQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withEditedStatus = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withEditedStatus; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*User)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.EditedStatus = []*VulnStatus{}
		}
		query.withFKs = true
		query.Where(predicate.VulnStatus(func(s *sql.Selector) {
			s.Where(sql.InValues(user.EditedStatusColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.user_edited_status
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_edited_status" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_edited_status" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.EditedStatus = append(node.Edges.EditedStatus, n)
		}
	}
