...
// CheckRuleMutation represents an operation that mutates the CheckRule nodes in the graph.
type CheckRuleMutation struct {
	config
	op              Op
	typ             string
	id              *int
	...
	oldValue        func(context.Context) (*CheckRule, error)
	predicates      []predicate.CheckRule
}

var _ ent.Mutation = (*CheckRuleMutation)(nil)

// checkruleOption allows management of the mutation configuration using functional options.
type checkruleOption func(*CheckRuleMutation)

// newCheckRuleMutation creates new mutation for the CheckRule entity.
func newCheckRuleMutation(c config, op Op, opts ...checkruleOption) *CheckRuleMutation {
	m := &CheckRuleMutation{
		config:        c,
		op:            op,
		typ:           TypeCheckRule,
...
// withCheckRuleID sets the ID field of the mutation.
func withCheckRuleID(id int) checkruleOption {
	return func(m *CheckRuleMutation) {
		var (
			err   error
			once  sync.Once
			value *CheckRule
		)
		m.oldValue = func(ctx context.Context) (*CheckRule, error) {
			once.Do(func() {
				if m.done {
					err = fmt.Errorf("querying old values post mutation is not allowed")
				} else {
					value, err = m.Client().CheckRule.Get(ctx, id)
				}
			})
			return value, err
		}
...
// withCheckRule sets the old CheckRule of the mutation.
func withCheckRule(node *CheckRule) checkruleOption {
	return func(m *CheckRuleMutation) {
		m.oldValue = func(context.Context) (*CheckRule, error) {
			return node, nil
		}
		m.id = &node.ID
	}
...
// Client returns a new `ent.Client` from the mutation. If the mutation was
// executed in a transaction (ent.Tx), a transactional client is returned.
func (m CheckRuleMutation) Client() *Client {
	client := &Client{config: m.config}
	client.init()
	return client
}
...
// Tx returns an `ent.Tx` for mutations that were executed in transactions;
// it returns an error otherwise.
func (m CheckRuleMutation) Tx() (*Tx, error) {
	if _, ok := m.driver.(*txDriver); !ok {
		return nil, fmt.Errorf("ent: mutation is not running in a transaction")
	}
	tx := &Tx{config: m.config}
...
// ID returns the ID value in the mutation. Note that the ID is only available
// if it was provided to the builder or after it was returned from the database.
func (m *CheckRuleMutation) ID() (id int, exists bool) {
	if m.id == nil {
		return
	}
	return *m.id, true
...
func (m *CheckRuleMutation) SetSeverityID(id int) {
	m.severity = &id
}

// ClearSeverity clears the "severity" edge to the Severity entity.
func (m *CheckRuleMutation) ClearSeverity() {
	m.clearedseverity = true
}

// SeverityCleared reports if the "severity" edge to the Severity entity was cleared.
func (m *CheckRuleMutation) SeverityCleared() bool {
	return m.clearedseverity
}

// SeverityID returns the "severity" edge ID in the mutation.
func (m *CheckRuleMutation) SeverityID() (id int, exists bool) {
	if m.severity != nil {
...
func (m *CheckRuleMutation) SeverityIDs() (ids []int) {
...
func (m *CheckRuleMutation) ResetSeverity() {
	m.severity = nil
	m.clearedseverity = false
}

// Where appends a list predicates to the CheckRuleMutation builder.
func (m *CheckRuleMutation) Where(ps ...predicate.CheckRule) {
	m.predicates = append(m.predicates, ps...)
}

// Op returns the operation name.
func (m *CheckRuleMutation) Op() Op {
	return m.op
}

// Type returns the node type of this mutation (CheckRule).
func (m *CheckRuleMutation) Type() string {
	return m.typ
}
...
// AddedFields returns all numeric fields that were incremented/decremented during
// this mutation.
func (m *CheckRuleMutation) AddedFields() []string {
	return nil
}
...
// with the given name. The second boolean return value indicates that this field
// was not set, or was not defined in the schema.
func (m *CheckRuleMutation) AddedField(name string) (ent.Value, bool) {
	return nil, false
}
...
// the field is not defined in the schema, or if the type mismatched the field
// type.
func (m *CheckRuleMutation) AddField(name string, value ent.Value) error {
	switch name {
	}
	return fmt.Errorf("unknown CheckRule numeric field %s", name)
}

// ClearedFields returns all nullable fields that were cleared during this
// mutation.
func (m *CheckRuleMutation) ClearedFields() []string {
	return nil
}

// FieldCleared returns a boolean indicating if a field with the given name was
// cleared in this mutation.
func (m *CheckRuleMutation) FieldCleared(name string) bool {
	_, ok := m.clearedFields[name]
	return ok
}
...
// ClearField clears the value of the field with the given name. It returns an
// error if the field is not defined in the schema.
func (m *CheckRuleMutation) ClearField(name string) error {
	return fmt.Errorf("unknown CheckRule nullable field %s", name)
}
...
// AddedEdges returns all edge names that were set/added in this mutation.
func (m *CheckRuleMutation) AddedEdges() []string {
	edges := make([]string, 0, 1)
	if m.severity != nil {
		edges = append(edges, checkrule.EdgeSeverity)
	}
	return edges
}
...
// AddedIDs returns all IDs (to other nodes) that were added for the given edge
// name in this mutation.
func (m *CheckRuleMutation) AddedIDs(name string) []ent.Value {
	switch name {
	case checkrule.EdgeSeverity:
		if id := m.severity; id != nil {
			return []ent.Value{*id}
		}
	}
...
// RemovedEdges returns all edge names that were removed in this mutation.
func (m *CheckRuleMutation) RemovedEdges() []string {
	edges := make([]string, 0, 1)
	return edges
}
...
// RemovedIDs returns all IDs (to other nodes) that were removed for the edge with
// the given name in this mutation.
func (m *CheckRuleMutation) RemovedIDs(name string) []ent.Value {
	switch name {
	}
	return nil
}
...
// ClearedEdges returns all edge names that were cleared in this mutation.
func (m *CheckRuleMutation) ClearedEdges() []string {
	edges := make([]string, 0, 1)
...
// EdgeCleared returns a boolean which indicates if the edge with the given name
// was cleared in this mutation.
func (m *CheckRuleMutation) EdgeCleared(name string) bool {
	switch name {
	case checkrule.EdgeSeverity:
		return m.clearedseverity
	}
...
// ClearEdge clears the value of the edge with the given name. It returns an error
// if that edge is not defined in the schema.
func (m *CheckRuleMutation) ClearEdge(name string) error {
	switch name {
	case checkrule.EdgeSeverity:
		m.ClearSeverity()
		return nil
	}
	return fmt.Errorf("unknown CheckRule unique edge %s", name)
}

// ResetEdge resets all changes to the edge with the given name in this mutation.
// It returns an error if the edge is not defined in the schema.
func (m *CheckRuleMutation) ResetEdge(name string) error {
	switch name {
	case checkrule.EdgeSeverity:
		m.ResetSeverity()
		return nil
	}
	return fmt.Errorf("unknown CheckRule edge %s", name)
}
